...
function genericPrintNoParens(path, options, print) {
    ...
    case "ImportDeclaration":
        ...
        if (n.specifiers &&
            ...
            const ImportSpecifierCount = (
                path.stack[path.stack.length - 1]
                .specifiers
                .filter(sp => namedTypes.ImportSpecifier.check(sp))
                .length
            );
            path.each(function(specifierPath) {
                ...
                if (namedTypes.ImportDefaultSpecifier.check(value) ||
                    ...
                    if (!foundImportSpecifier) {
                        ...
                        parts.push(
                          ImportSpecifierCount > 1 ? "{" : (options.objectCurlySpacing ? "{ " : "{")
                        );
                    }
                    parts.push(
                        ImportSpecifierCount > 1 ? '\n' : '',
                        ImportSpecifierCount > 1 ? print(specifierPath).indent(options.tabWidth) : print(specifierPath),
                        ImportSpecifierCount > 1 ? ',' : ''
                    );
                }
            }, "specifiers");
